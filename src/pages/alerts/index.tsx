...
import { getAlerts, deleteAlertById } from 'apiSdk/alerts';
import { AlertInterface } from 'interfaces/alert';

type ColumnType = ColumnDef<AlertInterface, unknown>;
...
export function AlertListPage(props: AlertListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AlertInterface>>(
    () => `/alerts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AlertInterface) => {
    if (hasAccess('alert', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/alerts/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/alerts/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('alert', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/alerts/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('alert', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="alert" />
        </Flex>

        {hasAccess('alert', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/alerts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

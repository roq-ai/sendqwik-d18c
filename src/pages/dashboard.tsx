...
import { getAlerts } from 'apiSdk/alerts';
import { AlertInterface } from 'interfaces/alert';
import { getDashboards } from 'apiSdk/dashboards';
import { DashboardInterface } from 'interfaces/dashboard';

import { AlertListPage as ShowCaseList } from 'pages/alerts';

function DashboardPage() {
  const { session } = useSession();

  const campaignStat = useSWR<PaginatedInterface<CampaignInterface>>(
    () => `/campaigns`,
    () => getCampaigns({ limit: 1, offset: 0 }),
  );

  const alertStat = useSWR<PaginatedInterface<AlertInterface>>(
    () => `/alerts`,
    () => getAlerts({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Campaigns',
      result: campaignStat?.data?.totalCount,
      isLoading: campaignStat.isLoading,
    },
    {
      title: 'Alerts',
      result: alertStat?.data?.totalCount,
      isLoading: alertStat.isLoading,
    },
    {
      title: 'Dashboards',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`alerts`} passHref legacyBehavior>
                  <Button
